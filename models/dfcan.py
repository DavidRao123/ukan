...
class DFCAN(nn.Module):
    def __init__(self, n_channels=1, n_classes=1, scale=1, size_psc=128): #size_psc：crop_size input_shape：depth
        super().__init__()
        self.input=nn.Sequential(nn.Conv2d(n_channels, 64, kernel_size=3, stride=1, padding=1),
                                       nn.GELU(),)
        n_ResGroup=4
        ResGroups=[]
        ...
        self.conv_sigmoid=nn.Sequential(nn.Conv2d(64, n_classes, kernel_size=3, stride=1, padding=1),
                                       nn.Sigmoid())
